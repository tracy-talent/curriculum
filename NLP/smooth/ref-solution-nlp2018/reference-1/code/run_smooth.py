...
class BertForSmooth(PreTrainedBertModel):
...
class DataProcessor(object):
    ...
    def get_train_examples(data_dir):
        ...
        examples = []
        for line in codecs.open(os.path.join(data_dir, 'smooth.train'), 'r', encoding='utf-8'):
            tokens = line.strip('\n').split('\t')
            examples.append(InputExample(tokens[0], int(tokens[1])))
        return examples
    ...
    def get_dev_examples(data_dir):
        ...
        examples = []
        for line in codecs.open(os.path.join(data_dir, 'smooth.dev'), 'r', encoding='utf-8'):
            tokens = line.strip('\n').split('\t')
            examples.append(InputExample(tokens[0], int(tokens[1])))
        return examples
    ...
    def get_test_examples(data_dir):
        ...
        ids, examples = [], []
        for line in codecs.open(os.path.join(data_dir, 'test_v3.txt'), 'r', encoding='utf-8'):
            tokens = line.strip('\n').split('\t')
            ids.append(tokens[0])
            examples.append(InputExample(tokens[1]))
        return ids, examples
...
def main():
    ...
    if args.do_train:
        ...
        model.train()
        for _ in trange(start_epoch, args.num_train_epochs, desc="Epoch"):
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
                ...
                sw.add_scalar('loss', loss.clone().cpu().data.numpy(), global_step)
                ...
                if (step + 1) % args.gradient_accumulation_steps == 0:
                    ...
                    optimizer.zero_grad()
                    global_step += 1
                if global_step > 800 and (step + 1) % 50 == 0:
                    dev_predictions, _ = do_predict(dev_dataloader, model, device)
                    model.train()
                    precision, recall, f1, _ = precision_recall_fscore_support(dev_labels, dev_predictions, average='binary')
    ...
    if args.do_predict:
        ...
        writer = codecs.open(os.path.join(args.output_dir, "test.predict-%d" % global_step), 'w', encoding='utf-8')
        for _id, label, class_proba in zip(ids, predictions, class_probas):
            writer.write(_id+'\t'+str(label)+'\t'+str(class_proba)+'\n')
        writer.close()
