...
def output(freqset, k_item_freq, frequency_threshold, k, dataset_path_name):
    ...
    :return: None
    """
    if not os.path.exists(os.path.join(OUTPUT_PATH, dataset_path_name + '_advanced_2')):
        os.makedirs(os.path.join(OUTPUT_PATH, dataset_path_name + '_advanced_2'))
    with open(os.path.join(OUTPUT_PATH, dataset_path_name + '_advanced_2/freq-' + str(k) + '-itemsets.txt'), 'w') as outfile:
        for key in freqset:
            keystr = ','.join(key)
