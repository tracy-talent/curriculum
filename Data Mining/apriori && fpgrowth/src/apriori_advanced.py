...
def output(freqset, k_item_freq, frequency_threshold, k, dataset_path_name):
    ...
    :return: None
    """
    if not os.path.exists(os.path.join(OUTPUT_PATH, dataset_path_name + '_advanced')):
        os.makedirs(os.path.join(OUTPUT_PATH, dataset_path_name + '_advanced'))
    with open(os.path.join(OUTPUT_PATH, dataset_path_name + '_advanced/freq-' + str(k) + '-itemsets.txt'), 'w') as outfile:
        for key in freqset:
            keystr = ','.join(key)
