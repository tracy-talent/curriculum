...
if not os.path.exists(os.path.join(config['path']['IMAGES'], 'algs_on_single_dataset')):
    os.makedirs(os.path.join(config['path']['IMAGES'], 'algs_on_single_dataset'))
pdf = PdfPages(os.path.join(config['path']['IMAGES'], 'algs_on_single_dataset/algsondatasets.pdf'))
data = []
...
for i in range(10):
    ...
    fig.tight_layout()
    plt.savefig(f"../imgs/algs_on_single_dataset/{fpath[fpath.rfind('/')+1:fpath.rfind('.')]}.png")
    pdf.savefig(fig)
